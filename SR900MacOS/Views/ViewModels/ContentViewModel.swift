...
class ContentViewModel: ObservableObject {
    ...
    // Start at 0 so initial layout can stabilize, then set to hidden offsets in init
    @Published var rectangle2Offset: CGFloat = 0
    @Published var rectangle3Offset: CGFloat = 0
    @Published var rectangle4Offset: CGFloat = 0
    ...
    private struct AnimationConstants {
        // panel widths (keep in sync with SlidingPanelsContainer)
        static let graphWidth: CGFloat = 607
        static let profilesWidth: CGFloat = 410
        static let settingsWidth: CGFloat = 410
        ...
        static let hiddenOffset: CGFloat = -(graphWidth + 20)
    }
    
    // MARK: - Init
    init() {
        // Delay setting hidden offsets until after initial layout to avoid one-frame overflow.
        DispatchQueue.main.async {
            self.rectangle2Offset = AnimationConstants.hiddenOffset
            self.rectangle3Offset = AnimationConstants.hiddenOffset
            self.rectangle4Offset = AnimationConstants.hiddenOffset
        }
    }
